...
> 将文字故事转化为精美短剧视频的 AI 创作工具（使用扣子积分调用模型版）
...
图像生成支持 `reference_images` 参数，可以传入多个人物的参考图：

- 在人物管理中为每个角色上传/生成正面参考图
- 生成分镜时，系统会自动获取所有出场人物的参考图
- 所有参考图作为输入，让 AI 在一张图中生成多个角色

#### 视频连续性方案

视频生成支持首尾帧连续性，确保相邻分镜之间的视觉无缝衔接：

**生成顺序：** 先生成所有分镜图片，再批量生成视频
...
这样每个视频的结尾画面与下一个视频的开头画面完全一致，实现无缝过渡。

> **注意**：需要在 Bot 中配置支持首尾帧的视频生成 Skill（如 seedance 1.5 pro）
