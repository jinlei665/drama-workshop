...
# 复制 node_modules (解析符号链接)
echo ""
echo "[5/7] 复制依赖 (这可能需要几分钟)..."
mkdir -p $TEMP_DIR/app/node_modules
cp -rL node_modules/* $TEMP_DIR/app/node_modules/ 2>/dev/null || true
...
# 简易启动脚本 (不安装依赖)
cat > $TEMP_DIR/start-quick.bat << 'EOF'
@echo off
cd /d "%~dp0app"
set PORT=5000
set HOSTNAME=0.0.0.0
...
# 简易启动脚本 (Linux)
cat > $TEMP_DIR/start-quick.sh << 'EOF'
#!/bin/bash
cd "$(dirname "$0")/app"
export PORT=5000
export HOSTNAME="0.0.0.0"
node node_modules/next/dist/bin/next start
EOF
chmod +x $TEMP_DIR/start-quick.sh
...
1. 复制环境变量模板：
   - Windows: `copy app\.env.example app\.env`
   - Linux: `cp app/.env.example app/.env`

2. 编辑配置文件，填入必要的 API 密钥：
...
2. 尝试运行 `start.bat`（Windows）或 `start.sh`（Linux）重新安装依赖

### Q: 启动报错 "EADDRINUSE: address already in use :::5000"
A: 端口 5000 已被占用，请关闭占用该端口的程序，或修改启动脚本中的 PORT 变量
...
echo "  3. Windows: 运行 start-quick.bat"
echo "     Linux:   运行 ./start-quick.sh"
echo "========================================"
