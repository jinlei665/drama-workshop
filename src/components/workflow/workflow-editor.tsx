  ...
  const getNodePortPosition = useCallback((
    ...
    if (type === 'input') {
      ...
      return {
        ...
        y: node.position.y + HEADER_HEIGHT + PORT_PADDING_Y + index * PORT_ROW_HEIGHT + 10
      }
    } else {
      // 输出端口在输入端口下方，有分隔线，右侧
      return {
        x: node.position.x + NODE_WIDTH - PORT_PADDING_X - PORT_DOT_RADIUS,
        y: node.position.y + HEADER_HEIGHT + inputSectionHeight + DIVIDER_HEIGHT + PORT_PADDING_Y + index * PORT_ROW_HEIGHT + 10
      }
    }
  }, [])
