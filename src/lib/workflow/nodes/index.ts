...
async function getFfmpegPathForNode(): Promise<string> {
  ...
  if (ffmpegPath) {
    try {
      ...
      console.log('[WorkflowNodes] 使用用户配置的 FFmpeg:', ffmpegPath)
      return ffmpegPath
    } catch (error) {
      console.warn(`[WorkflowNodes] 用户配置的 FFmpeg 路径 "${ffmpegPath}" 无效，尝试系统 FFmpeg`)
    }
  }
  ...
  try {
    ...
    if (lines.length > 0) {
      ...
      try {
        ...
        console.log('[WorkflowNodes] 使用系统 FFmpeg:', systemPath)
        return systemPath
      } catch {
        console.warn('[WorkflowNodes] 系统 FFmpeg 无效')
      }
    }
  } catch {
    console.warn('[WorkflowNodes] 无法从系统 PATH 检测 FFmpeg')
  }
...
### 1. 适度的场景过渡
- **重要的环境/氛围描写**：如果环境变化对剧情有重要影响，可以单独作为一个分镜
